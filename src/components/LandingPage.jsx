...
export default function LandingPage({ issues, setCurrentScreen, setSelectedIssueId }) {
  ...
  return (
    <div className="bg-slate-50 min-height-full">
      {/* Hero Section */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white rounded-2xl shadow-xl border border-slate-100 p-6 sm:p-8">
          <div className="flex items-center space-x-4 p-2 md:border-r border-slate-100">
            <div className="bg-tnblue-50 text-tnblue-700 p-4 rounded-xl">
              <Camera className="h-8 w-8" />
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Citizen Reports Logged</p>
              <p className="text-3xl font-black text-tnblue-900">{totalCitizenReports}</p>
              <p className="text-xs text-slate-400">Total individual complaints filed</p>
            </div>
          </div>

          <div className="flex items-center space-x-4 p-2 md:border-r border-slate-100">
            <div className="bg-tngreen-50 text-tngreen-700 p-4 rounded-xl">
              <CheckCircle2 className="h-8 w-8" />
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Issues Resolved</p>
              <p className="text-3xl font-black text-tngreen-700">{totalResolved}</p>
              <p className="text-xs text-tngreen-600 font-medium">
          ...
          <div className="flex items-center space-x-4 p-2">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Active Wards</p>
              <p className="text-3xl font-black text-slate-800">{activeWardsCount}</p>
              <p className="text-xs text-slate-400">Theni municipal wards covered</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">How the Platform Works</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
          ...
          <div className="hidden md:block absolute top-1/3 left-1/8 right-1/8 h-0.5 bg-slate-200 -z-10"></div>

          {/* Step 1 */}
          <div className="flex flex-col items-center text-center bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative">
            <span className="absolute -top-4 bg-tnblue-700 text-white font-bold w-8 h-8 rounded-full flex items-center justify-center border-4 border-slate-50 shadow">1</span>
            <div className="bg-tnblue-50 text-tnblue-800 p-4 rounded-full mb-4">
              <Camera className="h-8 w-8" />
            </div>
            <h3 className="font-bold text-lg text-slate-800 mb-2">1. Snap & Upload</h3>
          ...
          {/* Step 2 */}
          <div className="flex flex-col items-center text-center bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative">
            <span className="absolute -top-4 bg-tnblue-700 text-white font-bold w-8 h-8 rounded-full flex items-center justify-center border-4 border-slate-50 shadow">2</span>
            <div className="bg-tngreen-50 text-tngreen-700 p-4 rounded-full mb-4">
              <Brain className="h-8 w-8" />
            </div>
            <h3 className="font-bold text-lg text-slate-800 mb-2">2. AI Detects</h3>
            <p className="text-xs text-slate-500">
          ...
          {/* Step 3 */}
          <div className="flex flex-col items-center text-center bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative">
            <span className="absolute -top-4 bg-tnblue-700 text-white font-bold w-8 h-8 rounded-full flex items-center justify-center border-4 border-slate-50 shadow">3</span>
            <div className="bg-yellow-50 text-yellow-700 p-4 rounded-full mb-4">
              <UserCheck className="h-8 w-8" />
            </div>
            <h3 className="font-bold text-lg text-slate-800 mb-2">3. Ward Assigned</h3>
            <p className="text-xs text-slate-500">
          ...
          {/* Step 4 */}
          <div className="flex flex-col items-center text-center bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative">
            <span className="absolute -top-4 bg-tnblue-700 text-white font-bold w-8 h-8 rounded-full flex items-center justify-center border-4 border-slate-50 shadow">4</span>
            <div className="bg-green-50 text-green-700 p-4 rounded-full mb-4">
              <CheckCircle2 className="h-8 w-8" />
            </div>
            <h3 className="font-bold text-lg text-slate-800 mb-2">4. Track Resolved</h3>
            <p className="text-xs text-slate-500">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-4 sm:p-6 rounded-2xl border border-slate-100 shadow-md flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Showing active and resolved problems across Theni wards</p>
              </div>
              <button 
                onClick={() => setCurrentScreen('dashboard')}
                className="text-xs font-bold text-tnblue-700 hover:text-tnblue-800 flex items-center gap-0.5"
              >
            ...
            <div className="h-72 w-full rounded-xl overflow-hidden border border-slate-200 relative">
              ...
              <div className="absolute bottom-3 left-3 bg-white/95 px-2 py-1.5 rounded-lg border border-slate-250 z-20 shadow-sm text-[10px] space-y-1 flex flex-col">
                <span className="font-bold text-slate-800 mb-0.5">Map Pins key:</span>
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-red-600"></span> Reported / பதிவானது</span>
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-orange-650" style={{backgroundColor: '#ea580c'}}></span> Assigned / ஒதுக்கப்பட்டது</span>
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-sky-600"></span> In Progress / வேலை நடக்கிறது</span>
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-green-600"></span> Solved / தீர்க்கப்பட்டது</span>
              </div>
            </div>
          </div>

          {/* Recent Activity Right (1 column) */}
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-md flex flex-col justify-between">
            <div>
              <h3 className="font-extrabold text-xl text-slate-900 mb-1">Recent Reports</h3>
              <p className="text-xs text-slate-500 mb-6">Latest complaints submitted by citizens</p>
              
              <div className="space-y-4">
                {recentIssues.map((issue) => {
                  const catInfo = CATEGORIES[issue.category];
                  let statusBg = 'bg-red-100 text-red-700 border-red-200';
                  if (issue.status === 'Assigned') statusBg = 'bg-orange-100 text-orange-700 border-orange-200';
                  if (issue.status === 'In Progress') statusBg = 'bg-sky-100 text-sky-700 border-sky-200';
                  if (issue.status === 'Solved') statusBg = 'bg-green-100 text-green-700 border-green-200';
                  ...
                  return (
                    <div 
                      ...
                      className="group flex gap-3 p-3 rounded-xl border border-slate-100 hover:border-tnblue-100 hover:bg-slate-50 cursor-pointer transition-all duration-200"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-extrabold text-xs text-slate-800 truncate">{catInfo?.label || issue.category}</span>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-[10px] text-slate-400">{issue.location.ward} • {issue.location.address.split(',')[0]}</span>
                          <span className="text-[10px] text-slate-400 font-medium">{issue.reportedDate}</span>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <button
              onClick={() => setCurrentScreen('dashboard')}
              className="mt-6 w-full py-3 bg-tnblue-50 text-tnblue-800 text-sm font-bold rounded-xl hover:bg-tnblue-100 transition-colors duration-150 flex items-center justify-center space-x-1.5"
            >
