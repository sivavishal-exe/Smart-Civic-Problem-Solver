...
export default function StatusDashboard({ issues, setCurrentScreen, setSelectedIssueId }) {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col relative overflow-hidden bg-slate-100">
      
      {/* Mobile Toggle Bar */}
      <div className="md:hidden flex bg-white border-b border-slate-200 divide-x divide-slate-200">
        <button
          onClick={() => setMobileView('map')}
          className={`flex-1 py-3 text-center text-sm font-bold transition-all ${
            mobileView === 'map' ? 'bg-tnblue-50 text-tnblue-800 border-b-2 border-tnblue-800' : 'text-slate-500'
          }`}
        >
          Map View / வரைபடம்
        </button>
        <button
          ...
          className={`flex-1 py-3 text-center text-sm font-bold transition-all ${
            mobileView === 'list' ? 'bg-tnblue-50 text-tnblue-800 border-b-2 border-tnblue-800' : 'text-slate-500'
          }`}
        >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`w-full md:w-96 flex flex-col bg-white border-r border-slate-200 z-20 transition-all duration-300 ${
          ...
          <div className="p-4 border-b border-slate-200 space-y-3">
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-50 border border-slate-350 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-tnblue-500 focus:outline-none"
              />
            ...
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2 bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200 text-xs font-bold text-slate-700 transition-colors"
            >
            ...
            {showFilters && (
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-3 space-y-2.5 animate-fadeIn">
                <div>
                ...
                return (
                  <div
                    ...
                    className={`p-4 flex gap-3 cursor-pointer hover:bg-slate-50 transition-colors ${
                      activeIssueId === issue.id ? 'bg-tnblue-50 border-l-4 border-tnblue-700' : ''
                    }`}
                  >
                    <div className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 border border-slate-200 bg-slate-100">
                      <img src={issue.photoUrlBefore} alt={issue.category} className="w-full h-full object-cover" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-xs font-bold text-slate-800 truncate">{catVal?.label || issue.category}</span>
                      ...
                      <div className="flex items-center justify-between mt-2.5 text-[9px] text-slate-400">
                        ...
                        {issue.duplicateCount > 0 && (
                          <span className="bg-orange-50 text-orange-650 px-1 rounded font-bold">
        ...
        <div className={`flex-1 relative z-10 ${
          ...
          {activeIssue && (
            <div className="absolute bottom-4 left-4 right-4 md:left-auto md:right-4 md:top-4 md:bottom-auto w-auto md:w-96 bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-205 z-25 p-4 sm:p-5 animate-slideUp max-h-[85vh] md:max-h-[90vh] overflow-y-auto flex flex-col justify-between">
              
              {/* Header */}
              <div className="flex justify-between items-start mb-3 pb-3 border-b border-slate-100">
                <div>
                  <div className="flex items-center gap-1.5">
                    <span className="font-extrabold text-sm text-tnblue-800">{activeIssue.id}</span>
                    <span className="text-[10px] text-slate-400">•</span>
                    <span className="text-xs font-bold text-slate-700">{activeIssue.location.ward}</span>
                  </div>
                ...
                <button
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-800 p-1.5 rounded-full transition-colors"
                >
              ...
              <div className="space-y-4 flex-1">
                ...
                {activeIssue.status === 'Solved' && activeIssue.photoUrlAfter ? (
                  <div className="space-y-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">Before / After Comparison</span>
                    <div className="grid grid-cols-2 gap-2 h-28 rounded-lg overflow-hidden border border-slate-200">
                      <div className="relative">
                ...
                {/* Description */}
                <div>
                  <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block mb-0.5">Description</span>
                ...
                {/* Ward address */}
                <div className="flex items-start gap-1.5 text-xs text-slate-500 bg-slate-50 p-2.5 rounded-xl border border-slate-150">
                ...
                {activeIssue.duplicateCount > 0 && (
                  <div className="bg-orange-50 border border-orange-150 rounded-xl p-2.5 flex items-center space-x-2 text-xs">
                    <span className="bg-orange-500 text-white font-black px-2 py-0.5 rounded text-[10px]">{activeIssue.duplicateCount + 1}</span>
                    <span className="font-bold text-orange-850">Citizens Reported This Near Site</span>
                  </div>
                )}

                {/* Timeline Stepper */}
                <div className="py-2.5">
                  <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block mb-3.5">Status Progress</span>
                  <div className="flex items-center justify-between relative px-2">
                    {/* Progress Connecting Line */}
                    <div className="absolute top-3.5 left-6 right-6 h-0.5 bg-slate-200 -z-10"></div>
                    ...
                    {["Reported", "Assigned", "In Progress", "Solved"].map((step, idx) => (
                      <div key={step} className="flex flex-col items-center relative">
                        ...
                        <span className="text-[9px] font-bold text-slate-500 mt-1.5">{step}</span>
                      </div>
                    ))}
                  </div>
                </div>
